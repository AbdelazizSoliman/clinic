module Checkout
  class Readiness
    ...
    def initialize(user:, cart:, address:, payment_method: "cash")
      @user = user
      @cart = cart
      @address = address
    ...
    def call
      ...
      if @cart
        ...
        errors << "توجد كمية أكبر من المخزون المتاح" if @cart.items.includes(:product).any? { |item| item.quantity > item.product.stock_quantity }
      end
      ...
      errors << "اختر طريقة دفع تجريبية" if @payment_method.blank?
