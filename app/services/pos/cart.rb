module Pos
  class Cart
    ...
    def add(product:, quantity: 1)
      ...
      return failure(@sale, "لا يمكن استخدام منتج من مؤسسة أخرى") unless Operations::TenantGuard.same_organization?(@sale, @actor, product)
      ...
      PosSale.transaction do
        ...
        return failure(@sale, "الكمية المتاحة غير كافية") if requested > product.available_to_sell_quantity(@sale.branch)
    ...
    def update(item:, quantity:)
      ...
      return failure(@sale, "تعارض مؤسسة بند نقطة البيع") unless Operations::TenantGuard.same_organization?(@sale, @actor, item, item.product)
      ...
      return failure(@sale, "الكمية المتاحة غير كافية") if quantity > item.product.available_to_sell_quantity(@sale.branch)
      PosSale.transaction do
